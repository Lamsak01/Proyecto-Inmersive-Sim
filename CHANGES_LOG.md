...
- **Orientación Dinámica del Jugador:** 
    ...
    - Sin embargo, al transicionar directamente al interior de las casas (`House1Interior.tscn`), el motor detectará el destino y forzará al jugador a aparecer mirando hacia arriba (`up`), de espaldas a la cámara, simulando que acaba de entrar por la puerta.

## ⚡ Rendimiento de Transiciones
- **Corrección de Cuasi-Deadlocks (Congelamiento):** Se revirtió temporalmente la carga asíncrona de Background en hilos secundaria a través del `SceneTeleporter` debido a un cuello de botella de inicialización térmica y lógica (`ResourceLoader`). Se re-estableció el flujo directo y determinístico que evita crasheos y bloqueos totales en hardware especifico durante las recargas del árbol gráfico.
