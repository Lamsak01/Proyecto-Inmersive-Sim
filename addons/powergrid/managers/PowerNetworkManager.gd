...
var _transformers: Array[Node] = [] # Transformer type not yet confirmed, using Node for now or duck typing if class unavailable
...
func register_component(c: Node) -> void:
	...
	if c is PowerPort:
		return # Ports registered separately via register_port
...
func _bfs_from_port(start_port: PowerPort) -> void:
	...
	# Use a typed dictionary or just rely on port's energized state if purely for power propagation?
	# The original code used a visited dict.
	# However, if we set energized=true, we can use that as visited check if we clear it first (which we do in recalc_network).
	...
	while not q.is_empty():
		...
		# Propagate to owner component (Load/Transformer)
		var owner = p.get_owner_component()
		if owner:
			# Handle Transformer (pass-through)
